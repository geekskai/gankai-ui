<template>
  <div style="position: relative">
    <textarea
      @input="handleInput"
      v-bind="$props"
      class="gk__limit--textarea"
      :class="{'gk__limit-over': isOver}"
      :value="currentValue"
      >
    </textarea>
    <span class="gk__limit--text">
      {{isOver ? '已超出' : '还可以输入'}}<span class="gk__limit--num" :class="{'gk__limit--num-over': isOver}">{{num}}</span>个字
    </span>
  </div>
</template>
<script>
